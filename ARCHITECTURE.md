...
- **Caching Strategy (Redis):**
  - **Real-Time Quotes:** Key: QUOTE:{EXCHANGE_ID}:{SYMBOL}.
  - **Exchange Config:** Key: CONFIG:{EXCHANGE_ID}.
...
Steps required _before_ Terraform can run (Chicken and Egg scenarios).
...
APP\_SP\_ID=$(az ad sp list \--filter "appId eq '$APP_ID'" \--query "\[0\].id" \-o tsv)  
...
az rest \--method POST \\  
 ...
 \--body "{\\"principalId\\": \\"$APP\_SP\_ID\\", \\"resourceId\\": \\"$GRAPH_SP_ID\\", \\"appRoleId\\": \\"$ROLE_ID\\"}"
